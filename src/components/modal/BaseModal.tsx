...
export const BaseModal = (props: BaseModalProps) => {
    const { isOpen, onSubmit, onClose, title, children, disableSubmitBtn, styleTitle } = props;
                ...
                <div className="fixed inset-0 z-10">
                    <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div className="inline-block text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                            <div className="flex px-6 py-4 bg-white rounded-lg">
                                <BaseText bold locale size={20} className={classNames("w-full font-bold text-left text-darkNight900", styleTitle)}>{title}</BaseText>
                                <CloseOutlined onClick={closeModal} className="text-2xl text-gray-500 cursor-pointer" />
                            </div>
                            <div className="px-6 py-4 overflow-auto bg-white " style={{ maxHeight: '70vh' }}>
                                {children}
                            </div>
