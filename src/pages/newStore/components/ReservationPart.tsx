...
export const ReservationPart = (props: IProps) => {
    const { dataBanking, dataMeetAndCash, dataMeetAndTransfer, dataMeetAndCard, onSubmitBank, onChangeMeetAndCash, onChangeMeetAndTransfer, onChangeMeetAndCard } = props;
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className='flex flex-col gap-2'>
                    <Checkbox checked={meetAndCash} onChange={handleChangeMeetAndCash}>
                        <BaseText medium locale>만나서 현금결제</BaseText>
                    </Checkbox>
                    <Checkbox checked={meetAndTransfer} onChange={handleChangeMeetAndTransfer}>
                        <BaseText medium locale>만나서 계좌이체</BaseText>
                    </Checkbox>
                    {
                        meetAndTransfer && (
                            <div className="flex justify-between p-3 rounded-lg bg-darkNight50" onClick={() => { setIsShowModal(true) }}>
                                <div className="flex gap-2">
                                    {dataBanking?.bankNumber && <img src={dataBanking?.bankImage} alt={dataBank?.bankName} className="w-6 h-6" />}
                                    <BaseText size={16} className={classNames(dataBanking?.bankNumber ? "" : 'text-darkNight300')}>{dataBanking?.bankNumber || t('은행선택')}</BaseText>
                                </div>
                                <DownOutlined />
                            </div>
                        )
                    ...
                    <Checkbox checked={meetAndCard} onChange={handleChangeMeetAndCard}>
                        <BaseText medium locale>만나서 카드결제</BaseText>
                    </Checkbox>
                </div>
            </div>
            <BaseModal2
                ...
                title={
                    dataBank?.bankId ?
                        <div className="flex gap-2">
                            <img src={dataBank?.bankImage} alt={dataBank?.bankName} className="w-6 h-6" />
                ...
                <div>
                    {dataBank?.bankId ? (
                        ...
                        LIST_BANKING.map((item, index) => (
                            <div key={index} className="flex items-center gap-2 p-3 cursor-pointer" onClick={() => {
                                setDataBank({
                                ...
                                <img src={item.imageBank} alt={item.nameBank} className="w-6 h-6" />
