...
export const ModalSelectOpeningHours = (props: IProps) => {
  ...
  const handleRemoveTime = (indexToRemove: number) => {
    setListBreakTime(
      listBreakTime.filter((item, index) => index !== indexToRemove)
    );
  };

  const handleRemoveTimeWeekend = (indexToRemove: number) => {
    setListBreakTimeWeekend(
      listBreakTimeWeekend.filter((item, index) => index !== indexToRemove)
    );
  };
      ...
      <div className="flex items-center gap-6 mb-4">
        ...
        >
          <BaseText
            locale
            bold
            className={classNames(!isSelected ? "text-primary" : "")}
          >
            휴무일이 있어요
          </BaseText>
        </div>
              ...
              <>
                <div className="flex gap-2 mb-2">
                  <div className="flex items-center flex-1 gap-2">
                    ...
                    <TimePicker.RangePicker format="HH:mm" className="flex-1" />
                  </div>
                  <div
                    onClick={() => {
                      setIsAlwaysOpen(!isAlwaysOpen);
