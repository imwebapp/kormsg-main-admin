...
const UserManage = () => {
  ...
  const isFormDataValid = () => {
    for (const key in formDataCreateUser) {
      if (key !== "avatar" && !formDataCreateUser[key as keyof typeof formDataCreateUser]) {
        return false;
      }
    }
    return true;
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          setFormDataCreateUser({ ...formDataCreateUser, avatar: reader.result });
        } else {
      ...
      <div className={classNames('flex overflow-hidden')} style={{ height: 'calc(100vh - 71px)' }}>
        <div className={classNames('w-1/6 border-r border-darkNight100 p-6')}>
          <div className={classNames('flex items-center justify-between mb-4')}>
            <BaseText bold locale size={16} className="">User group</BaseText>
            <PlusOutlined className={classNames('text-xl cursor-pointer')} onClick={handleOpenModalCreateGroup} />
          </div>
          <div className={classNames('overflow-y-auto')} style={{ height: 'calc(100% - 40px)' }}>
            {listUserGroup.map((item, index) => {
              const checkSelected = groupSelected && groupSelected.id === item.id;
              return (
                <div className={classNames('flex items-center gap-1 py-2 mb-2 cursor-pointer')} onClick={() => handleClickGroup(item)}>
                  {checkSelected && <CheckOutlined className={classNames('text-dayBreakBlue500 text-xl')} />}
                  <BaseText
                    medium
                    locale
                    size={14}
                    className={classNames(checkSelected ? 'text-dayBreakBlue500' : 'pl-6')}
                  >
                    {item.name}
                  </BaseText>
                  <span
                    className={classNames('', checkSelected ? 'text-dayBreakBlue500 font-medium' : 'font-medium')}
                  >
                    ({item.count})
                  </span>
                </div>
        ...
        <div className={classNames('w-5/6 p-6 flex flex-col gap-6')} style={{ height: 'calc(100vh - 71px)' }}>
          <BaseText
            bold
            locale
            size={24}
          ...
          <div className={classNames('flex flex-row justify-between items-center')}>
            ...
            <div className={classNames('flex gap-4')}>
              <CustomButton
                ...
                className={classNames('flex items-center justify-center px-4 py-6')}
                onClick={() => { }}
              >
                Create users bulk
              </CustomButton>
      ...
      >
        <div className={classNames(' flex flex-col gap-4')}>
          <div className={classNames(' flex items-center justify-center')}>
            ...
            />
            <label htmlFor="avatarInput" className={classNames('relative inline-block')}>
              <img
                src={formDataCreateUser.avatar || Images.avatarEmpty}
                className={classNames('w-20 h-20 rounded-full mx-auto mb-5 cursor-pointer')}
                alt="Avatar"
              />
          ...
          <BaseInputSelect
            ...
            options={(TypeUser || []).map((item) => ({ value: item.id, label: item.name }))}
          />
          <BaseInputSelect
            title="Group"
            required
            ...
            options={(listUserGroup || []).map((item) => ({ value: item.id, label: item.name }))}
          />
