...
const UserDetail = () => {
  ...
  const [optionSelected, setOptionSelected] = useState('shopInformation');
    ...
    <>
      <div className={classNames('flex overflow-hidden')} style={{ height: 'calc(100vh - 71px)' }}>
        <div className={classNames('w-1/5 border-r border-darkNight100')}>
          <div className={classNames('flex flex-col gap-2 border-b border-darkNight100 p-6')}>
            <img
            ...
            <div className={classNames('flex flex-row items-center gap-3')}>
              <BaseText bold size={28} className={classNames('text-darkNight900')}>
            ...
            <div className={classNames('flex flex-row items-center gap-1')}>
              <BaseText bold className={classNames('text-purple flex px-4 py-2 items-center bg-goldenPurple50')}>
                {formDataCreateUser.type}
              </BaseText>
              <BaseText medium className={classNames('text-darkNight900 flex px-4 py-2 items-center bg-darkNight50')}>
          ...
          </div>
          {
            listOption.map((item: { title: string, value: string }, index) => {
              const checkSelected = optionSelected && optionSelected === item.value;
              return (
                <div className={classNames('flex items-center gap-1 px-6 py-3 mb-2 cursor-pointer border-l-2 ', checkSelected ? ' bg-dayBreakBlue50 border-l-2 border-dayBreakBlue500' : '')} onClick={() => handleClickOption(item)}>
                  <BaseText
                    medium
                    locale
                    size={16}
                    className={classNames(checkSelected ? 'text-dayBreakBlue500' : 'text-darkNight500')}
                  >
                    {item.title}
                  </BaseText>
                </div>
        ...
        <div className={classNames('w-4/5 flex flex-col gap-6')} style={{ height: 'calc(100vh - 71px)' }}>
          {
          ...
          }
          {
            optionSelected === 'shopInformation' ? (
              <div className={classNames('flex flex-col gap-6')}>
                <BaseText bold size={24} className={classNames('text-darkNight900')}>
                  Shop information
                </BaseText>
              </div>
            ) : null
